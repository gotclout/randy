...
#include <stdlib.h>
#include <sys/stat.h>
#include <ctype.h> //implicit declaration of isspace warning
...
int tokenize(char **pcomm, char* args[], char** outfile, char** infile, int* flag)
{
  ...
  else
  {
    ...
    for(token = strtok(comm, " "); token !=0 && argc < MAX_LEN; token=strtok(0," ") )
    {
      ...
      {
        token=strtok(0," ");                        //set the token to null ended, don't care what is after the outfile operator
      ...
      else if(strcmp(args[argc],"&")==0)           //if ampersand encountered in command you are calling
      {
        ...
        argc++;                                    //if encountered no operators keep on incrementing argc
    }
  }
...
int system(const char *comm)
{
  ...
  int p = 0;
  for(  ; p < len && comm[p] != ' '; p++)   //for loop to go through the command
  {
  ...
  tokenize(&co,args,&outfile,&infile,&flag);  //calling the tokenize function you pass *co instead of comm.
  ...
  if((child= fork())==0)     //if forked child process
  {
    if(redirectout==1)       //if redirecting the command to an output file
    {
    ...
    execlp(a,"",cc, NULL);       //calling the execlp command and passing the arguments to it.
